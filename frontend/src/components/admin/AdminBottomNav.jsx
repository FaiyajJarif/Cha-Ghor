...
// feel like different kinds of thing -- a module is not less important because
// it came later in the array.
//
// So every IN-SCOPE module is here and the row scrolls sideways. (Modules
// flagged `soon` in lib/adminNav.js are filtered out below, so the count is the
// length of that array minus those -- do not hard-code it here again; the
// previous version of this comment said "all nine" and was wrong twice over
// within a month.) Arrows appear ONLY when
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        >
          {/* Out-of-scope modules are OMITTED here rather than greyed. The
              sidebar has room to show "Soon" as information; a 76px tab on a
              phone does not, and a dead tab would still cost a scroll slot in a
              row that already scrolls. `soon` is set in lib/adminNav.js. */}
          {ADMIN_NAV.filter((n) => !n.soon).map(({ key, label, path, icon: Icon }) => (
            <NavLink
              key={key}
              to={path}
              end={path === "/admin"}
